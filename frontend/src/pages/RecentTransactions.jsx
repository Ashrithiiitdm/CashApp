...
import { 
  ...
  UserNameIcon,
  ReceiptIcon 
} from '../components/Icons'; // Ensure you have a generic Receipt/Bill icon
    ...
    <div className="min-h-screen w-full bg-[#1581BF] flex items-center justify-center p-4 font-sans">
      ...
      <div className="bg-[#f8f9fd] w-11/12 max-w-[420px] min-h-[750px] rounded-[40px] shadow-2xl relative flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          {/* Date Header (Static for demo) */}
          {filteredTransactions.length > 0 && (
             <h3 className="text-gray-500 text-xs font-semibold mt-2">January 2, 2026</h3>
          )}
            ...
            filteredTransactions.map((txn) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                    {/* Bill Icon (only for debits/stores) */}
                    {txn.type === 'debit' && (
                        <ReceiptIcon className="w-5 h-5 text-blue-400" />
                    )}

                    {/* Amount Coloring: Green for Credit (+), Black for Debit */}
